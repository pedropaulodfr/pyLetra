...
def colar_link(e):
    Label(janela, text='Link: ').place(x=115, y=40)
    link = Entry(janela)
    link['width'] = 40
    link.place(x=150, y=40)
    link.focus_force()
    global url
    url = link
...
def search(e):
    ...
    if (music.get() != '') or (artist.get() != ''):
        ...
        if url.get() != '': # PROCURANDO MUSICA PELO LINK
            info = YouTube.encontrar(url.get())
            artista_link = info['artista']
            musica_link = info['musica']
...
def salvar(musica, artista, link, letra_music):
    # Criando arquivo txt
    arquivo = open('letras/' + musica + ' - ' + artista + '.txt', 'w')
...
def sobre():
    ...
    close['command'] = partial(fechar, janela_sobre)
...
# Botão colar link
btn_link = Button(janela, text='Colar link', activeforeground='white', activebackground='grey', cursor='hand2')
btn_link.place(x=333, y=120)
link = btn_link['command'] = partial(colar_link, 'nulo')
...
#btn_search['command'] = partial(search, 'nulo')
