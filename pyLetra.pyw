...
dia = 6
mes = 1
versao = 'Versão: 2.' + str(mes) + '.' + str(int((dia * mes)/10))
...
def search_off(a):
    def get_list(event):
        ...
        janela_letra(musicSave, seltext, janela)
...
def search(e):
    ...
    def translate():
        ...
        janela_letra(musicSave, musica + ' - ' + artista, janela)
...
def youtube(musica, artista):
    ...
    musica = urllib.parse.quote(musica)
    artista = urllib.parse.quote(artista)

    driver.get(URL + f"results?search_query={musica}+-+{artista}")
